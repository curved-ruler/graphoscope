...
SRC+=" "
SRC+=$(find "$BASE_PATH/apps/synth" -name '*.cc')
...
g++ -o $SYNTH_OUTPUT $COMPILER_OPTIONS $DEBUG_OPTIONS "-I$BASE_PATH" "-I$THIRD_PATH" "-I$THIRD_PATH/stb-2020-06-20" "-I$THIRD_PATH/sokol-2024-08-21" "-I$GLAD_INC" `pkg-config --cflags glfw3` $SRC -L"$BASE_PATH/dist/bin-l" -lgsgl -lcrcc -lasound `pkg-config --static --libs glfw3`
