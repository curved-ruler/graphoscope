...
SRC+=" "
SRC+=$(find "$BASE_PATH/apps/planetmaker" -name '*.cc')
...
g++ -o $PLANETM_OUTPUT $COMPILER_OPTIONS $DEBUG_OPTIONS "-I$BASE_PATH" "-I$THIRD_PATH" "-I$THIRD_PATH/stb-2020-06-20" "-I$GLAD_INC" `pkg-config --cflags glfw3` $SRC -L"$BASE_PATH/dist/bin-l" -lgsgl -lcrcc `pkg-config --static --libs glfw3`
