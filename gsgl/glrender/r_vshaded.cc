...
void r_vshaded::setup(cr::render_mode* rm)
{
    settings = rm;
    
    std::string vs = pc.process(settings->shader_dir,  "v-shaded.glsl.v");
...
void r_vshaded::init_render(unsigned int w, unsigned int h)
{
...
void r_vshaded::render(const cr::camera& cam, const cr::mat4& modeltr, const cr::rrr_buffers& bufs)
{
#ifdef C_GSGL_DEBUG
    if (status != 1)
    {
    ...
    glEnableVertexAttribArray(obj::NOR_BUF_ID);
    ...
    glVertexAttribPointer(obj::NOR_BUF_ID, 3, GL_FLOAT, GL_FALSE, 9*sizeof(float), BUFFER_OFFSET(6 * sizeof(float)));
    ...
    cr::vec3 L(1.0f,1.0f,1.0f);
    L.normalize();
    succ &= vf->set_uniform_v3 ("light", L);
    succ &= vf->set_uniform_v3 ("vpos",  cam.pos);
    ...
    glDisableVertexAttribArray(obj::NOR_BUF_ID);
}
